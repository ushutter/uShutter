// app.cpp
// Orchestration/state machine/main loop for uShutter.
// Display and capture internals are split into Display.* and Capture.*
...
// =================================================
// SINGLE mode sensor selection (compile-time)
//
// To avoid ambiguity, choose which physical sensor is used for
// MODE_SINGLE_EXPOSURE.
//
//   1 -> use s1 (SENSOR_B / D3)
//   2 -> use s2 (SENSOR_A / D2)
//
// Default below matches your current usage (single mode uses s1).
// =================================================
#define SINGLE_MODE_SENSOR 1

// =================================================
// Frontend auto-detect via A1
// Hardware requirement:
// - Use ONE fixed pull-up on A1 to VCC: 10k (external resistor)
// - Each frontend selects one pulldown to GND:
//     FE1 = 1k   -> map to Dual
//     FE2 = 2.2k -> map to Single
//     FE3 = 4.7k -> reserved
//     FE4 = 10k  -> reserved

#define FRONTEND_DETECT_PIN A1

// ADC bucket thresholds for 10k pull-up scheme (10-bit ADC)
//   adc < 140          -> FE1 (1k)    -> Dual
//   140 <= adc < 255   -> FE2 (2.2k)  -> Single
//   255 <= adc < 420   -> FE3 (4.7k)  -> Reserved
//   adc >= 420         -> FE4 (10k)   -> Reserved
#define DETECT_TH_FE1_MAX 140
#define DETECT_TH_FE2_MAX 255
#define DETECT_TH_FE3_MAX 420
#define FRONTEND_DEBUG_SERIAL 1

// If you want to force a type, set to 1.
// Otherwise, auto-detect from A1 at boot.
#define FRONTEND_FORCE_MANUAL 0
SensorFrontend sensor_frontend = FRONTEND_FE1_DUAL;
...
// Idle level sampling (used for polarity-agnostic edge capture)
bool idle_high_s2 = true;
bool idle_high_s1 = true;
...
// Frontend detect (A1)
// =================================================
static uint16_t readAdcMedian(uint8_t pin, uint8_t n = 16) {
  pinMode(pin, INPUT_PULLUP);

  // Let the divider settle
  delay(50);

  // Dummy reads to flush ADC S/H capacitor history
  analogRead(pin);
  delay(2);
  analogRead(pin);
  delay(2);
...
static void debugFrontendDetect(uint16_t adc, SensorFrontend fe);

static SensorFrontend detectFrontendFromAx() {
  pinMode(FRONTEND_DETECT_PIN, INPUT);
  delay(50);

  uint16_t adc = readAdcMedian(FRONTEND_DETECT_PIN, 24);

  // FE mapping with fixed 10k pull-up on A1:
  // FE1 (1k)   -> Dual
  // FE2 (2.2k) -> Single
  // FE3 (4.7k) -> reserved
  // FE4 (10k)  -> reserved
  SensorFrontend fe;
  if (adc < DETECT_TH_FE1_MAX) {
    fe = FRONTEND_FE1_DUAL;
  } else if (adc < DETECT_TH_FE2_MAX) {
    fe = FRONTEND_FE2_SINGLE;
  } else if (adc < DETECT_TH_FE3_MAX) {
    fe = FRONTEND_FE3_RESERVED;
  } else {
    fe = FRONTEND_FE4_RESERVED;
  }
  debugFrontendDetect(adc, fe);
  return fe;
}

static void debugFrontendDetect(uint16_t adc, SensorFrontend fe) {
#if FRONTEND_DEBUG_SERIAL
  Serial.print(F("[FE] adc="));
  Serial.print(adc);
  Serial.print(F(" -> "));
  switch (fe) {
    case FRONTEND_FE1_DUAL: Serial.println(F("FE1 Dual")); break;
    case FRONTEND_FE2_SINGLE: Serial.println(F("FE2 Single")); break;
    case FRONTEND_FE3_RESERVED: Serial.println(F("FE3 Reserved")); break;
    case FRONTEND_FE4_RESERVED: Serial.println(F("FE4 Reserved")); break;
  }
#endif
}
...
void configureSensorFrontend() {
  ...
  MIN_EDGE_SPACING_US = 0;
}

// =================================================
// Sample idle (shutter-closed) logic level on each sensor pin.
// This makes edge capture robust to extra inversion stages on different PCB revisions.
// =================================================
void sampleIdleLevels() {
  // Assumption: when we're in READY (no shutter event), sensors sit at a stable idle level.
  idle_high_s2 = digitalRead(SENSOR_A);
  idle_high_s1 = digitalRead(SENSOR_B);
}
...
// =================================================
// UI helpers (moved to Display.cpp)
// =================================================
#include "Display.h"
...
void resetMeasurement() {
  ...
  noInterrupts();
  s2.t_open = s2.t_close = 0;
  s2.opened = false;
  s1.t_open = s1.t_close = 0;
  s1.opened = false;
  new_result = false;
  capture_done = false;
...
// Speed helpers
// =================================================
float speedFromTravelUs(unsigned long travel_us) {
...
// =================================================
float stabilityScoreFromCV(float cv_percent) {
...
// =================================================
// =================================================
// Display: single realtime (before table is full)
...
// - Long press : toggle mode (DUAL<->SINGLE)
// - Double click (only when FE=Dual and mode=SINGLE): toggle SINGLE sensor (s1/s2)
// =================================================
...
void setup() {
...
#else
  sensor_frontend = detectFrontendFromAx();
#endif
  ...
  do {
    ...
    int w = u8g2.getStrWidth(VERSION_STR);
    u8g2.drawStr(128 - w - RIGHT_MARGIN - 2, 62, VERSION_STR);
  } while (u8g2.nextPage());

  delay(2000);
  displayFrontendSplash();
  delay(2000);
  displayModeSplash();
  delay(2000);
  displayCentered("READY");
...
void loop() {
  ...
  // Pick the active sensor for SINGLE mode
  unsigned long single_open = singleUsesS1() ? s1_open : s2_open;
  unsigned long single_close = singleUsesS1() ? s1_close : s2_close;

  // Validate the primary exposure (SINGLE: selected sensor, DUAL: s2)
  unsigned long exp_s2 = 0;
  if (measure_mode == MODE_SINGLE_EXPOSURE) {
    if (single_close <= single_open) {
      enterError(5);
      return;
    }
    exp_s2 = single_close - single_open;
  } else {
    if (s2_close <= s2_open) {
      enterError(5);
      return;
    }
    exp_s2 = s2_close - s2_open;
  }
  ...
  if (measure_mode == MODE_DUAL_CURTAIN) {
    ...
    // Single mode realtime screen matches the dual layout
    updateDisplaySingle(exp_s2, err_s2, singleUsesS1());
  }
}

...
// ADC thresholds (for Rup=100k + internal pull up, equivilent 650 ohm):
//   74HC14: 100 /650ohm  -> ~ 10
//   74AHC14: 1K /650ohm  -> ~ 100
//   LM393: 100k/650ohm -> ~ 300


#define DETECT_TH_74HC14_MAX 100  // < 100  -> 74HC14
#define DETECT_TH_74AHC14_MIN 300   // 300~700 -> 74AHC14
#define DETECT_TH_74AHC14_MAX 700   // 300~700 -> 74AHC14
#define DETECT_TH_LM393_MIN 800   // >=800 -> LM393
...
static inline bool singleUsesS1() {
  return (SINGLE_MODE_SENSOR == 1);
}

static inline uint8_t singlePin() {
  return singleUsesS1() ? SENSOR_B : SENSOR_A;
}
...
// Frontend detect (A0)
// =================================================
static uint16_t readAdcAvg(uint8_t pin, uint8_t n = 16) {
  unsigned long sum = 0;

  // throw-away read after channel settle
  analogRead(pin);
...
static SensorFrontend detectFrontendFromAx() {
  ...
  //digitalWrite(FRONTEND_DETECT_PIN, LOW);

  // power / divider settle
  delay(50);

  //uint16_t adc = readAdcAvg(FRONTEND_DETECT_PIN, 24);
  ...
  // Example idea:
  // 74HC14  : adc <= TH_HC14_MAX
  // LM393   : TH_LM393_MIN <= adc <= TH_LM393_MAX
  // 74AHC14 : adc >= TH_AHC14_MIN
  // else    : default LM393

  if (adc <= DETECT_TH_74HC14_MAX) {
    return FRONTEND_74AHC14;
  }

  if (adc >= DETECT_TH_74AHC14_MIN && adc < DETECT_TH_74AHC14_MAX) {
    return FRONTEND_74HC14;
  }

  if (adc >= DETECT_TH_LM393_MIN) {
...
void displayModeSplash() {
  ...
  do {
    ...
    if(sensor_frontend == FRONTEND_LM393)
      f = "LMC393";
    else if(sensor_frontend == FRONTEND_74HC14)
      f = "74HC14";
    else
      f = "74AHC14";
    // = (sensor_frontend == FRONTEND_LM393) ? "LM393" : "74HC14";
